...
handle_cast({#apirequest{add_char = C}, From}, State) when C =/= undefined ->
  ...
  case mnesia:transaction(Create) of
    {atomic, Char} ->
      From ! api_pb:encode_apiresponse(#apiresponse{msg = "ok", char = Char});
    {aborted, _Reason} ->
...
handle_cast({#apirequest{get_char = C}, From}, State) when C =/= undefined ->
  ...
  case mnesia:transaction(Get) of
    {atomic, Chars} ->
      From ! api_pb:encode_apiresponse(#apiresponse{msg = "ok", char = Chars});
    {aborted, _Reason} ->
      From ! api_pb:encode_apiresponse(#apiresponse{msg = "error", info = "No characters found."})
  end,
